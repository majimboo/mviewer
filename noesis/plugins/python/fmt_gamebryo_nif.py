...
class NifObject:
	...
	def loadBSTriShape(self, bs):
		...
		self.vertFlags = bs.readUInt()
		if self.vertFlags == 0:
			return #possible todo
		self.unknown1 = bs.readUInt()
		...
		self.posOffset = 0
		self.uvOffset = 8 if self.vertSize >= 12 else -1 #not necessarily correct, can have normal without uv. should verify flags.
		self.normalOffset = 12 if self.vertSize >= 16 else -1
		self.tangentOffset = -1 #16
		self.colorOffset = -1 #looks dictated by 0x00000003
		#also seems to be dictated by vertFlags 0xF0000000, but we're playing it safe
...
class NifFile:
			...
			if bsShaderProp.bsTexSetID != NIF_INVALID_LINK_ID:
				...
				if len(bsTexSet.texNames) >= 5:
					...
					if self.nifHack == NIF_HACK_FO4:
						#no pbr at the moment, so just do some arbitrary scaling on the envmap
						envScale = 0.25
						fresnelScale = 0.5
	...
	def rpgCommitBsShapes(self, noeBones, bindCorrectionBones, noeMatList, noeTexList):
		...
		for triShape in self.bsTriShapes:
			...
			rapi.rpgBindPositionBufferOfs(triShape.vertData, noesis.RPGEODATA_HALFFLOAT, triShape.vertSize, triShape.posOffset)	
			if triShape.uvOffset >= 0:
				rapi.rpgBindUV1BufferOfs(triShape.vertData, noesis.RPGEODATA_HALFFLOAT, triShape.vertSize, triShape.uvOffset)
			if triShape.normalOffset >= 0:
				rapi.rpgBindNormalBufferOfs(triShape.vertData, noesis.RPGEODATA_UBYTE, triShape.vertSize, triShape.normalOffset)
			if triShape.tangentOffset >= 0:
				rapi.rpgBindTangentBufferOfs(triShape.vertData, noesis.RPGEODATA_UBYTE, triShape.vertSize, triShape.tangentOffset)
			if triShape.colorOffset >= 0 and useVertColors > 0:
				rapi.rpgBindColorBufferOfs(triData.geomColors, noesis.RPGEODATA_UBYTE, triShape.vertSize, triShape.colorOffset, 4)
			if isSkinned:
				rapi.rpgBindBoneIndexBufferOfs(triShape.vertData, noesis.RPGEODATA_UBYTE, triShape.vertSize, triShape.blendIdxOffset, 4)
				rapi.rpgBindBoneWeightBufferOfs(triShape.vertData, noesis.RPGEODATA_HALFFLOAT, triShape.vertSize, triShape.blendWgtOffset, 4)
...
def nifConstructModelFromNif(nif):
	...
	try:
		#some models may need special tangent/binormal treatment (which also means we can't use the fast construct path)
		useGeneratedBinormals = nif.nifHack == NIF_HACK_SPLATTERHOUSE
		if useGeneratedBinormals is True:
